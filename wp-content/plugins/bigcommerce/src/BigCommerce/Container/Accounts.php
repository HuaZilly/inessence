...
class Accounts extends Provider {
	...
	private function login( Container $container ) {
		...
		add_action( 'template_redirect', $this->create_callback( 'redirects', function () use ( $container ) {
            ...
            if ( is_user_logged_in() && in_array( $page, $account_pages ) ) {
                // custom function redirect to BigCommerce page after register
                $allowed_roles = array('editor', 'administrator', 'author');
                $user = wp_get_current_user();
                if( !array_intersect($allowed_roles, $user->roles ) ) {
                    wp_redirect( $container[ self::REGISTER ]->set_login_token_for_redirect(), 303 );
                    exit;
                }
            }
    ...
    private function logout( Container $container ) {
        add_filter( 'logout_redirect', $this->create_callback( 'logout_redirect', function ( $redirect_to, $requested_redirect_to, $user ) use ( $container ) {
            if( !(current_user_can('editor') || current_user_can('administrator') )) {
                $container[ self::REGISTER ] = function ( Container $container ) {
                    return new Registration_Handler($container[ Compatibility::SPAM_CHECKER ], $container[ Merchant::ONBOARDING_API ], $container[ Api::FACTORY ]->store());
                };
                wp_redirect( $container[ self::REGISTER ]->set_login_token_for_redirect( $user->ID, '/login.php?action=logout' ), 303 );
            }
            exit;
        }), 10, 3 );
    }
