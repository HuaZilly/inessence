...
function cbsb_activate( $plugin ) {
	if ( $plugin == 'calendar-booking/start-booking.php' ) {
		...
		if ( false === $connect_hash && false === $previous_connection ) {
			// Create new trial
			...
			if ( property_exists( $json, 'error' ) ) {
				...
				if ( isset( $json->data ) ) {
					...
					cbsb_api_request( 'account/options/plugin_deactivated', array(), 'DELETE' );
...
function cbsb_admin_activate_notice() {
	...
	if ( cbsb_is_connected() && !cbsb_is_activated() && !$notice_dismissed ) {
